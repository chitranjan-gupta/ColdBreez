    ...
    <div className="bg-white py-10">
      <div className="mx-auto grid max-w-2xl grid-cols-1 items-center px-4 gap-x-5 lg:grid-rows-2 lg:grid-cols-2 lg:max-w-7xl">
        <div className="order-2 md:order-1">
          <h2 className="text-5xl font-bold tracking-tight text-gray-900 md:text-4xl">{author.name ? <>{author.name}<CheckBadgeIcon className="h-10 w-10 inline text-blue-500" aria-hidden="true" /></> : <></>}</h2>
          <p className="mt-1 text-gray-500 text-xl font-bold">{author.bio ? author.bio : <></>}</p>
          ...
          <div className="mx-auto grid max-w-2xl grid-cols-1 gap-x-8 gap-y-5 border-t border-gray-200 pt-5 lg:mx-0 lg:max-w-none">
            {posts.map((post: {
              subcategories: any;
              categories: any;
              mainImage: SanityImageSource; _id: React.Key; publishedAt: string | Date; slug: { current: string; }; title: string; description: string;
            }) => (
              <article key={post._id} className="flex max-w-xl max-h-[220px] lg:max-h-[165px] overflow-hidden flex-row gap-2 items-center justify-between">
                <div className="flex max-w-xl flex-col items-start justify-between">
                  <div className="flex items-center justify-between flex-row gap-x-4 text-xs w-full">
                    <div className="flex items-start flex-col">
                      <time dateTime={post.publishedAt.toString()} className="text-gray-500 flex items-start flex-col gap-1">
                  ...
                  <div className="group relative">
                    <h3 className="mt-2 text-lg line-clamp-2 font-semibold leading-6 text-gray-900 group-hover:text-gray-600">
                      <Link href={`/news/${post.categories[0].toLowerCase()}/${post.subcategories[0].toLowerCase()}/${encodeURIComponent(
                        post.slug.current,
                      )}`} prefetch={false}>
                ...
                <div className="relative h-36 w-36 min-h-[144px] min-w-[144px]">
                  <Image src={post.mainImage ? urlFor(post.mainImage).url() : "/android-chrome-512x512.png"} alt={`${post.title}`} fill={true} className="rounded-lg bg-gray-100 w-full h-full" />
                </div>
              </article>
            ))}
          </div>
